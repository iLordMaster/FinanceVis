...
const Overview = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [incomeResponse, expenseResponse, accountsData, monthlyStats, incomeCategories, expenseCategories] = await Promise.all([
        TransactionApi.getTransactions({ type: 'INCOME', startDate: startOfYear, endDate: endOfToday }),
        TransactionApi.getTransactions({ type: 'EXPENSE', startDate: startOfYear, endDate: endOfToday }),
      ...
      [...(incomeCategories.categories || []), ...(expenseCategories.categories || [])].forEach(cat => {
        categoryMap.set(cat._id, cat);
      });

      // Map categories to transactions
      const incomeTransactions = (incomeResponse.transactions || []).map(t => ({
        ...t,
        categoryId: typeof t.categoryId === 'string' ? categoryMap.get(t.categoryId) : t.categoryId
      }));
      
      const expenseTransactions = (expenseResponse.transactions || []).map(t => ({
        ...t,
        categoryId: typeof t.categoryId === 'string' ? categoryMap.get(t.categoryId) : t.categoryId
      }));
      ...
      const uniqueIncomeCategories = new Set(
        incomeTransactions.map(t => t.categoryId?._id || t.categoryId).filter(Boolean)
      );
  ...
  const getFinancialTip = () => {
    const tips = [
      { icon: '📈', text: `You have ${metrics.incomeStreams} active income stream${metrics.incomeStreams !== 1 ? 's' : ''}. Diversification is key!` },
      { icon: '💡', text: 'Track your expenses regularly to identify areas where you can save more.' },
      { icon: '🎯', text: 'Set monthly budget goals to stay on track with your financial objectives.' },
      { icon: '💰', text: `Your net savings this year: $${metrics.netSavings.toFixed(2)}. Keep it up!` },
      { icon: '📊', text: 'Review your spending patterns monthly to make informed financial decisions.' }
    ];
    ...
    <div className="overview-container">
      ...
      <div className="metrics-grid">
        <MetricCard
          ...
          value={`$${metrics.totalIncome.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          icon={<FaDollarSign />}
          gradient="income"
        />
        <MetricCard
          title="Total Expenses (YTD)"
          value={`$${metrics.totalExpenses.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          icon={<FaWallet />}
          gradient="expense"
        />
        <MetricCard
          title="Net Savings"
          value={`$${metrics.netSavings.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          icon={<FaPiggyBank />}
          gradient="savings"
        />
        <MetricCard
      ...
      <div className="overview-middle">
        <DashboardCard title="Recent Activity" className="recent-activity-card">
          <RecentActivityList limit={5} onRefresh={refreshKey} />
        </DashboardCard>
        ...
        <DashboardCard title="Account Balance" className="account-balance-card">
          <div className="balance-display">
            ...
            <div className="balance-info">
              <div className="balance-amount">${accountBalance.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
              <div className="balance-label">Total across all accounts</div>
            </div>
          </div>
        </DashboardCard>

        <DashboardCard title="Financial Insight" className="tips-card">
