class BudgetController {
  ...
  async create(req, res) {
    try {
      ...
      if (err.message.includes('required') || err.message.includes('must be') || err.message.includes('exists')) {
  ...
  async update(req, res) {
    try {
      const result = await this.updateBudget.execute(req.params.id, req.user.id, req.body);
      res.json({
