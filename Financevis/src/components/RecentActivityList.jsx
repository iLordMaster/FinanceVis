...
const RecentActivityList = ({ limit = 5, onRefresh }) => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const [transactionResponse, incomeCategories, expenseCategories] = await Promise.all([
      ...
      if (transactionResponse.transactions && Array.isArray(transactionResponse.transactions)) {
        // Map category data to transactions
        const transactionsWithCategories = transactionResponse.transactions.map(transaction => ({
          ...transaction,
  ...
  return (
    <div className="recent-activity-list">
      {transactions.map((transaction, index) => (
        <div key={transaction._id || `transaction-${index}`} className="activity-item">
