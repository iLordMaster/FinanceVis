...
export default function DualLineChart() {
      ...
      try {
        if (timeRange === '1M' || timeRange === '3M') {
          ...
          for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
            const dayStr = d.toLocaleDateString('en-US', { day: '2-digit', month: 'short' }); // "01 Dec"
            dailyMap.set(dayStr, { month: dayStr, income: 0, expenses: 0, rawDate: new Date(d) });
          }
  ...
  const indicatorStyle = {
    transform: `translateX(${activeIndex * 42}px)`, // 40px width + 2px gap
  };
    ...
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "10px", paddingRight: "20px" }}>
        <div className="chart-time-selector">
          <div className="chart-time-selector-indicator" style={indicatorStyle}></div>
          {timeOptions.map((range) => (
            <button
              key={range}
              onClick={() => setTimeRange(range)}
              className={`chart-time-option ${timeRange === range ? 'active' : ''}`}
            >
              {range}
            </button>
          ))}
